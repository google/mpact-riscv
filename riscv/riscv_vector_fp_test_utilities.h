...
#include "util/floatops/roundedops.h"
...
constexpr char kFs1 = 4;

using ::util::floatops::detail::ScopedRoundingModeChange;
...
struct FPTypeInfo<float> {
  ...
  static bool IsNaN(T value) { return isnan(value); }
};
...
inline void FPCompare<float>(float op, float reg, int delta_position,
  ...
  if (!isnan(op) && !isinf(op) && delta_position < FPTypeInfo<T>::kSigSize) {
    T delta;
